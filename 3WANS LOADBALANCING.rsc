...
{
/ip firewall mangle
add action=mark-connection chain=prerouting dst-address-type=!local new-connection-mark=wan1_conn passthrough=yes per-connection-classifier=src-address-and-port:2/0 src-address=172.10.0.0/16
add action=mark-connection chain=prerouting dst-address-type=!local new-connection-mark=wan2_conn passthrough=yes per-connection-classifier=src-address-and-port:2/1 src-address=172.10.0.0/16
add action=mark-routing chain=prerouting connection-mark=wan1_conn new-routing-mark=to_wan1 passthrough=yes src-address=172.10.0.0/16
add action=mark-routing chain=prerouting connection-mark=wan2_conn new-routing-mark=to_wan2 passthrough=yes src-address=172.10.0.0/16



/ip firewall mangle
add action=mark-connection chain=prerouting dst-address-type=!local new-connection-mark=wan3_conn passthrough=yes per-connection-classifier=src-address-and-port:3/3 src-address=172.10.0.0/16
add action=mark-routing chain=prerouting connection-mark=wan3_conn new-routing-mark=to_wan3 passthrough=yes src-address=172.10.0.0/16
}
...
add action=masquerade chain=srcnat out-interface=WAN3 src-address=172.10.0.0/16
}

}

{
/interface load-balance add name=load-balance use-ip-firewall=yes inbound-interface-list=LAN outbound-interface-list=WAN protocol=ip
/interface load-balance set [ find default=yes ] direct-eth1-weight=3 direct-eth2-weight=1 direct-eth3-weight=1
