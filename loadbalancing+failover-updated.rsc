...
#Adding bridge and renaming interface
{
{
...
{
:local portsToBridge "ether4,ether5,ether6,ether7"
:foreach port in=[:toarray $portsToBridge] do={
    /interface bridge port add bridge=ether_LAN interface=$port
}
}
#Ip address LAN
{
...
add action=mark-connection chain=prerouting connection-mark=no-mark connection-state=new dst-address-type=!local in-interface=ether_LAN new-connection-mark=ISP1 passthrough=yes per-connection-classifier=both-addresses:3/0
add action=mark-connection chain=prerouting connection-mark=no-mark connection-state=new dst-address-type=!local in-interface=ether_LAN new-connection-mark=ISP2 passthrough=yes per-connection-classifier=both-addresses:3/2
add action=mark-connection chain=prerouting connection-mark=no-mark connection-state=new dst-address-type=!local in-interface=ether_LAN new-connection-mark=ISP3 passthrough=yes per-connection-classifier=both-addresses:3/1
...
add action=mark-routing chain=prerouting connection-mark=ISP3 in-interface=ether_LAN new-routing-mark=ISP3_table passthrough=yes
